@php
    ...
    $field['temporary'] = $field['temporary'] ?? false;
    $field['expiration'] = $field['expiration'] ?? 1;
    ...
    if (! function_exists('getDiskUrl')) {
        function getDiskUrl($field, $value) {
            try {
                ...
                try {
                    $origin = $field['temporary'] || $field['disk'] === config('filesystems.cloud') ? config('filesystems.disks.' . $field['disk'].'.url') : substr($disk->url('/'), 0, -1);
                }catch (Exception $e) {
                    // some drivers like S/FTP don't support getting URLs
                    Log::error($e);
                }
                $value = str_replace($origin ?? '', '', $value);
                $value = Str::of($value)->start('/');

                if ($field['temporary']) {
                    return $disk->temporaryUrl($value, now()->addMinutes((int) $field['expiration']));
                }
...
{{-- If a field type is shown multiple times on a form, the CSS and JS will only be loaded once --}}
    ...
    @push('crud_fields_styles')
        ...
        @bassetBlock('backpack/pro/fields/image-field.css')
        <style>
            .image .btn-group {
                margin-top: 10px;
            }
    ...
    @push('crud_fields_scripts')
        ...
        @bassetBlock('backpack/pro/fields/image-field.js')
        <script>
